...
<%
    ...
    EPerson e=(EPerson)request.getAttribute("eperson");
    ...
    boolean added = ((Boolean) request.getAttribute("added")).booleanValue();
...
<form method="get" class="btn-group" action="<%= request.getContextPath() %>/course-added">
<input type="hidden" name="collection" value="<%= collection.getHandle() %>" />
<input type="hidden" name="eperson" value="<%= e %>" />
<%  if(!added) { %>
<input type="submit" class="btn btn-default" name="submit-course" value="add to My courses"/>
<% } %>
...
<% 
   ...
   if(rating.size()!=0)
   {
	...
	r_value=rating.get(0);
	out.println("rated "+r_value.intValue()+"/"+5);
	r_value=r_value*20;
...
%>
<input type="hidden" id="rating_value" value="<%=r_value.intValue()-r_value.intValue()%10 %>" />
<div id="rated"></div>
<%  if (submit_button)
    { %>
