...
public class CollectionDAOImpl extends AbstractHibernateDSODAO<Collection> implements CollectionDAO
{
    ...
    @Override
    public List<Double> avg(Context context,Collection collection) throws SQLException { 
	 StringBuilder query = new StringBuilder();  
	//query.append("select c from Item i join i.collection c join i.rating r ");
       // return ((Long)createQuery(context, query.toString()).uniqueResult()).intValue();
	query.append("select avg(r.ratingValue) from Rating r join r.item i join i.collections col where col in (from Collection c where c.id = :collection_id ) group by col ");
	
	Query hibernateQuery = createQuery(context, query.toString());
	hibernateQuery.setParameter("collection_id", collection.getID()); 
	return hibernateQuery.list();
    }


@Override
    public List<Collection> findCollectionInMycourses(Context context , EPerson ePerson) throws SQLException {
        
	 StringBuilder query = new StringBuilder();
	query.append("SELECT DISTINCT col FROM Mycourse s join  s.collection col where s.ePerson in (from EPerson e where e.id = :eperson_id)");
	 Query hibernateQuery = createQuery(context, query.toString());
